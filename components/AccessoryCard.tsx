...
const AccessoryCard = ({accessory, type}: {
    ...
    return (
        <div onMouseEnter={() => setIsMouseOver(true)}
            ...
            <Link href={`/products/shoes/${accessory.accessoryId}`} target='_blank'>
                <div>
                    <Image src={accessory.thumbnail} alt=""
                           className="w-full md:w-[15rem] rounded-lg h-[17rem] bg-contain" width={2000}
                           height={2000}/>
                </div>
                <div className="mt-2 flex flex-col gap-1">
                    <div>
                        <h2 className="line-clamp-1 text-slate-400 font-bold text-sm capitalize">{accessory.name}</h2>
                    </div>
                    <div className="flex text-lg flex-row justify-start gap-2 items-center">
                        <Rating size={"medium"} readOnly value={accessory?.rating} precision={0.1}/>
                        <p className="font-medium">{accessory.rating}</p>
                    </div>
                    <div className="flex flex-row flex-wrap justify-between items-center">
                        <div className="relative w-fit">
                            <div className="h-[1.1px] absolute top-1/2 left-0 w-[4.2rem] bg-black"/>
                            <div className="flex flex-row items-center justify-center gap-3">
                                <h2 className="font-medium text-lg">රු {accessory.sellingPrice + 2000}</h2>
                                <p className="bg-red-600 text-sm p-[2px] rounded text-white">{-((2000 / (accessory.sellingPrice + 2000)) * 100).toFixed(2)}</p>
                            </div>
                        </div>
                        <div>
                            <h2 className="text-lg font-medium text-primary">රු {accessory.sellingPrice}</h2>
                        </div>
                    </div>
                </div>
            </Link>
            {addToCart && (
                <div className="flex flex-col mt-2 justify-center items-center">
                    <div className="flex flex-row flex-wrap gap-1 text-sm">
                        {accessory.sizes?.map((size: number, index: number) => (
